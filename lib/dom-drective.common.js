...
let startX,
  ...
  name: 'Loadedifcomplete',
  touch: false
}, {
  name: 'PhotosBackColor',
  touch: true,
  scroll: true
}, {
  name: 'observeHeight',
...
const Speed = {
  ...
  componentUpdated: (el, binding) => {
    ...
    let timer = requestAnimationFrame(function fn() {
      ...
      if (positive || negative) oldValue += speedNum; // else
      // oldValue = numer && cancelAnimationFrame(timer)

      if (typeof oldValue === 'number') StyleStr = `background-image: linear-gradient(to right,${color} ${oldValue}%, transparent 0);`;else if (typeof oldValue === 'string') StyleStr = `background-image: linear-gradient(to right,${color} ${oldValue}, transparent 0);`;
      el.style.cssText = StyleStr;
...
const StartCall = (Directive, value, DirectiveName, obj) => {
  if (!value) {
    console.warn(`No Function or No Value ? undefined`);
    return;
  }
  ...
  if (typeof value == "function") {
    ShowDirectiveArr.includes(Directive) ? value.call(undefined, Directive, ...afterOtion) : value.call(undefined, ...afterOtion);
    ...
    ShowDirectiveArr.includes(Directive) ? value.methods.call(undefined, ...option, Directive, ...afterOtion) : value.methods.call(undefined, ...option, ...afterOtion);
  }
};
...
let TouchStart = (callback, el, binding, e) => {
  ...
  if (SlideDirection.includes('LongTouch')) {
    ...
    el.LongTouchTime = setTimeout(() => {
      callback('LongTouch', e);
    }, 666);
...
let TouchMove = (callback, el, binding, e) => {
  ...
  if (SlideDirection.includes('PhotosBackColor')) {
    ...
    if ((direction == 1 || direction == 2) && scrollTop < 0) el.style.backgroundImage = `linear-gradient(${color} ${scrollTop * -1 + 2}px, #fff 0)`;else el.style.backgroundImage = ``;
  }
  ...
  const Move = () => {
    el.DistanceMove = true; // if( !SlideDirection.includes('Click') ) 
    //    e.stopPropagation()
  };

  const NoMove = () => {
    el.DistanceMove = false; // if( !SlideDirection.includes('Click') )
    //   e.stopPropagation()
  };
...
let ScrollFun = (callback, el, binding, e) => {
  ...
  if (SlideDirection.includes('PhotosBackColor')) {
    let color = binding && binding.value,
        scrollTop = el.scrollTop;
    if (scrollTop < 0) el.style.backgroundImage = `linear-gradient(${color} ${scrollTop * -1 + 2}px, #fff 0)`;else el.style.backgroundImage = ``;
  }
};
...
/******/ })()
;
//# sourceMappingURL=dom-drective.common.js.map